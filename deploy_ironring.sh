...
# n8n public domain (used in Traefik config — set to your real domain or IP later)
N8N_DOMAIN="n8n.yourdomain.com"
LETSENCRYPT_EMAIL="you@yourdomain.com"

# WireGuard config for netstack-Docker overlay
WG_SERVER_PRIVATE_KEY="$(wg genkey 2>/dev/null || echo 'REPLACE_WITH_WG_PRIVATE_KEY')"
WG_SERVER_PUBLIC_KEY="$(echo "$WG_SERVER_PRIVATE_KEY" | wg pubkey 2>/dev/null || echo 'REPLACE_WITH_WG_PUBLIC_KEY')"
WG_PORT=51820
...
for cmd in oci openssl jq curl; do
...
MY_IP=$(curl -s --max-time 5 https://api.ipify.org || curl -s --max-time 5 https://ifconfig.me || echo "")
if [ -z "$MY_IP" ]; then
  echo "⚠️  Could not auto-detect your public IP. SSH will be locked to 10.0.0.0/8 (internal only)."
...
SHAPE_CONFIG='{"ocpus":1,"memoryInGBs":6}'  # 1 OCPU / 6GB for A1 Always Free (4 shared across tenancy)

# Check if A1.Flex is available in the configured AD
...
if [ "$SHAPE_AVAILABLE" -gt 0 ]; then
  ...
  echo "   ✅ VM.Standard.A1.Flex is available — using Arm Ampere (Always Free)"
else
...
# Boot volume: OCI minimum is 50GB, Always Free max is 200GB total
# We use 50GB per instance (Always Free ceiling per volume)
BOOT_VOL_GB=50
...
oci iam policy create \
  ...
  --description "Least-privilege policy: NetworkAdmins manage networking + compute in $COMPARTMENT_NAME" \
  --compartment-id "$ROOT_COMPARTMENT_OCID" \
  --statements "[
...
# ─────────────────────────────────────────────
echo ""
echo "🔑 Generating API keys for $USER_NAME..."
KEY_DIR="$HOME/.oci/${USER_NAME}"
mkdir -p "$KEY_DIR"
chmod 700 "$KEY_DIR"

openssl genrsa -out "$KEY_DIR/oci_api_key.pem" 2048 2>/dev/null
chmod 600 "$KEY_DIR/oci_api_key.pem"
openssl rsa -pubout \
  -in  "$KEY_DIR/oci_api_key.pem" \
  -out "$KEY_DIR/oci_api_key_public.pem" 2>/dev/null

oci iam user api-key upload \
  --user-id "$USER_OCID" \
  --key-file "$KEY_DIR/oci_api_key_public.pem" >/dev/null
...
# Write named profile (does NOT overwrite DEFAULT)
...
oci network nsg rules add \
  ...
  --security-rules "[
    ...
    {\"direction\":\"INGRESS\",\"protocol\":\"17\",\"isStateless\":false,
     ...
     \"description\":\"WireGuard VPN overlay\"},
    ...
    {\"direction\":\"INGRESS\",\"protocol\":\"6\",\"isStateless\":false,
     \"source\":\"10.0.0.0/16\",\"sourceType\":\"CIDR_BLOCK\",
     \"tcpOptions\":{\"destinationPortRange\":{\"min\":3306,\"max\":3306}},
     \"description\":\"MySQL from VCN (internal services only)\"},
    {\"direction\":\"EGRESS\",\"protocol\":\"all\",\"isStateless\":false,
     \"destination\":\"0.0.0.0/0\",\"destinationType\":\"CIDR_BLOCK\",
...
if [ -z "$IMAGE_OCID" ] || [ "$IMAGE_OCID" = "null" ]; then
  echo "❌ Could not locate Ubuntu 24.04 image in region $REGION. Aborting."
  exit 1
fi
...
# Shared Docker install base (all 3 instances)
DOCKER_CLOUD_INIT_BASE='#!/bin/bash
set -e
apt-get update -y
...
# n8n-Docker: Docker + Traefik + n8n via Docker Compose
N8N_CLOUD_INIT=$(cat <<CLOUDINIT
#!/bin/bash
set -e
${DOCKER_CLOUD_INIT_BASE}
ufw allow 80/tcp
ufw allow 443/tcp
ufw --force enable

# Create Traefik + n8n compose stack
...
# ubuntu-Node: Docker + hardening only
...
# netstack-Docker: Docker + WireGuard server
...
# Add peer entries here for n8n-Docker and ubuntu-Node
# [Peer]
...
# Run WireGuard UI container for easy peer management (accessible on WG subnet only)
docker run -d \
  --name wg-easy \
  --cap-add NET_ADMIN \
  --cap-add SYS_MODULE \
  -e WG_HOST=\$(curl -s https://api.ipify.org) \
  -e WG_DEFAULT_ADDRESS="${WG_SERVER_IP}/24" \
  -e WG_DEFAULT_DNS=1.1.1.1 \
  -e WG_ALLOWED_IPS="${WG_SUBNET}" \
...
launch_instance() {
  ...
  echo "→ Launching: $name (pub_ip=$assign_pub_ip)..."

  # Build shape config JSON conditionally
  local shape_config_arg=""
  if [ "$USE_SHAPE_CONFIG" = true ]; then
    shape_config_arg="--shape-config {\"ocpus\":1,\"memoryInGBs\":6}"
  fi

  # Base64-encode cloud-init
  local userdata_b64
...
wait_for_instance() {
  ...
  echo "⚠️  $name did not reach RUNNING within 10 minutes. Check OCI console."
  return 1
}
...
#  SSH CONFIG BLOCK
# ─────────────────────────────────────────────
SSH_CONFIG_BLOCK=$(cat <<SSHCONF
# ── IronRing SSH Config ──────────────────────────────
# Append this to ~/.ssh/config  (or source it for OCI Cloud Shell)

Host ironring-n8n
  HostName ${N8N_PUBLIC_IP}
  User ubuntu
  IdentityFile ${SSH_PUB_KEY_PATH%%.pub}
...
#  (standalone, executable in OCI Cloud Shell terminal)
# ─────────────────────────────────────────────
CLOUD_SHELL_SETUP=$(cat <<CLOUDSHELL
#!/bin/bash
# ═══════════════════════════════════════════════════════════
...
# Cloud Shell uses ~/.ssh — generate key if not present
if [ ! -f ~/.ssh/id_rsa ]; then
  ssh-keygen -t rsa -b 4096 -f ~/.ssh/id_rsa -N "" -C "oci-cloudshell-ironring"
  echo ""
  echo "⚠️  NEW KEY GENERATED. Add this public key to your OCI instances:"
  cat ~/.ssh/id_rsa.pub
  echo ""
  echo "   In OCI Console → Compute → Instances → [instance] → Console Connections"
  echo "   Or add via: ssh ubuntu@\$N8N_PUBLIC_IP"
  echo "   Then run:   echo \"\$(cat ~/.ssh/id_rsa.pub)\" >> ~/.ssh/authorized_keys"
fi
...
echo ""
echo "   Or direct: ssh -i ~/.ssh/id_rsa ubuntu@\${N8N_PUBLIC_IP}"
CLOUDSHELL
)

CLOUD_SHELL_FILE="$(dirname "$0")/ironring_cloud_shell_setup.sh"
printf '%s\n' "$CLOUD_SHELL_SETUP" > "$CLOUD_SHELL_FILE"
...
echo ""
echo "  Instances:"
printf "  %-22s %-46s public: %s\n" "$N8N_INSTANCE_NAME"    "$N8N_INSTANCE_OCID"    "${N8N_PUBLIC_IP:-<pending>}"
printf "  %-22s %-46s private: %s\n" "$UBUNTU_INSTANCE_NAME"  "$UBUNTU_INSTANCE_OCID"  "${UBUNTU_PRIVATE_IP:-10.0.2.x}"
printf "  %-22s %-46s private: %s\n" "$NETSTACK_INSTANCE_NAME" "$NETSTACK_INSTANCE_OCID" "${NETSTACK_PRIVATE_IP:-10.0.2.y}"
echo ""
...
echo "  SSH config backed up to:     ~/.ssh/config.bak.*"
echo "  Cloud Shell setup script:    $CLOUD_SHELL_FILE"
echo ""
cat <<NOTES
═══════════════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════════════
  ...
  [ ] DNS: Point ${N8N_DOMAIN} → ${N8N_PUBLIC_IP:-<n8n public IP>}
  [ ] n8n will be live at https://${N8N_DOMAIN} once Traefik gets a Let's Encrypt cert
      (cert issues automatically on first request — DNS must resolve first)

  [ ] WireGuard peers: SSH into netstack-Docker and run:
        sudo wg set wg0 peer <PEER_PUBKEY> allowed-ips 10.8.0.2/32
      Or use the wg-easy UI at http://10.8.0.1:51821 (WG tunnel only)

  [ ] Rotate SSH: Remove 0.0.0.0/0 from security lists if your IP changes;
      update NoMercy-Sec and StreetPatrol-NSG in OCI Console.

  [ ] OCI Vault: Move API keys and WireGuard private key into OCI Vault
      for production secret management.

  [ ] Monitoring: Enable VCN Flow Logs in OCI → Logging → Log Groups
      for full network audit trail.

  [ ] Snapshots: After first successful boot, snapshot all 3 boot volumes
      via OCI Console → Block Storage → Boot Volumes.

  WireGuard private key (store securely, do not share):
  ${WG_SERVER_PRIVATE_KEY}

═══════════════════════════════════════════════════════════════
NOTES

echo "🧾 Done. Use OCI CLI profile [${USER_NAME}] for future operations."
echo "   export OCI_CLI_PROFILE=${USER_NAME}"
