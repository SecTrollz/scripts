...
| 🟡 (fixed) | `multihop_proxy_setup.sh` | Linux, systemd, root | Installer for a local `:8080` forwarding proxy (Go source embedded in the script) that chains Tor + optionally I2P SOCKS hops with a randomized set of public Proxynova HTTP CONNECT proxies (order reshuffled every rebuild, every 2 minutes), MITMs the traffic itself to rebuild it as one of two paired browser identities (matching TLS ClientHello *and* HTTP header set/order together, chosen once per request — a Firefox fingerprint wearing Chrome's headers is a combination no real browser produces, so the two are never picked independently), resolves hostnames over a rotating pool of real DoH resolvers, fingerprints its outbound TLS with `utls`, optionally AES-GCM-encrypts request/response bodies between itself and a matching peer, and can add random per-request delay to blunt timing correlation. Installs as a hardened systemd service. **Was broken (three separate bugs, each fatal on its own):** (1) the script wrote the Go source directly to the binary path and `chmod +x`'d the source file — never compiled it, so the proxy could never actually start. (2) the TLS fingerprinting code referenced `utls.HelloChrome_112`, which doesn't exist in the `utls` library at all. (3) all twenty-two of its `dnscry.pt-odoh-<city>` DNS-over-HTTPS resolver hostnames are NXDOMAIN in real DNS (confirmed live against a public resolver) — every one was fictional, so hostname resolution, and therefore every request to a domain name rather than a literal IP, would have failed 100% of the time even after (1) and (2) were fixed. Fixed by building a real Go module and compiling it (`go mod init/tidy` + `go build`), swapping the TLS profile constant for the real `utls.HelloChrome_120`, and replacing the resolver list with seven independently-operated DoH endpoints (Cloudflare, Quad9, OpenDNS, Digitale Gesellschaft, LibreDNS, AdGuard, Mullvad) verified live with real RFC 8484 queries. **Also fixed, on top of just making it run:** Go's own `net/http` was itself a distinguishing signature independent of any of the above — `http.Header.Write()` always serializes headers in sorted alphabetical order, which no real browser does, and the `utls` ClientHello presets advertise HTTP/2 support in ALPN that the code never actually spoke, a mismatch a server could notice. Both are fixed by hand-writing the outbound HTTP/1.1 request in real browser header order (verified against a live HTTPS server, byte-for-byte, not just compiled) instead of going through `net/http`'s request writer, and constraining each profile's ALPN offer to `http/1.1` only so the handshake never promises a protocol the code doesn't speak. The always-on `X-Forwarded-For` header (spoofed with private RFC1918 addresses drawn from a fixed pool) was removed outright rather than fixed: real browser traffic never sends that header at all, and private IPs inside it are a stronger, more automated-looking tell than simply not sending it — the opposite of the anonymity goal. Also added `ReadWritePaths=/var/log` to the systemd hardening block so the `-log` flag can actually write under `ProtectSystem=strict`, and a small per-(browser-profile, host) connection pool for the upstream leg, since dialing a fresh TCP+TLS connection through the whole hop chain for every single request is itself unusual traffic behavior (real browsers keep sockets open) on top of being slow. **Read before trusting it as your only layer:** the non-Tor/I2P hops are random *public* HTTP proxies scraped from a third-party PAC file — anyone running one of those can see your traffic before it re-enters Tor, so this is defense-in-depth on top of Tor, not a Tor replacement; it still needs a real Tor (and, if enabled, I2P) daemon already running locally, and the DoH resolvers are ordinary DoH, not genuine HPKE-based Oblivious DoH, so no single one of them sees *every* lookup but each one individually still sees the lookups it's picked for. |
