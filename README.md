...
| 🟡 (fixed) | `multihop_proxy_setup.sh` | Linux, systemd, root | Installer for a local `:8080` forwarding proxy (Go source embedded in the script) that chains Tor + optionally I2P SOCKS hops with a randomized set of public Proxynova HTTP CONNECT proxies (order reshuffled every rebuild, every 2 minutes), MITMs the traffic itself to strip and rebuild headers (fake per-hop `X-Forwarded-For` chain, rotating UA), resolves hostnames over ODoH against a rotating pool of `dnscry.pt` relays, fingerprints its outbound TLS with `utls` (Firefox/Chrome ClientHello), optionally AES-GCM-encrypts request/response bodies between itself and a matching peer, and can add random per-request delay to blunt timing correlation. Installs as a hardened systemd service. **Was broken:** the script wrote the Go source directly to the binary path and `chmod +x`'d the source file — never compiled it, so the proxy could never actually start (`Type=simple` + `ExecStart` on raw `package main` text is an `ENOEXEC`, not a running proxy). Separately, the TLS fingerprinting code referenced `utls.HelloChrome_112`, which doesn't exist in the `utls` library at all (never has). Fixed by building a real Go module and compiling it (`go mod init/tidy` + `go build`, verified to compile clean with `go vet`), swapping the TLS profile constant for the real `utls.HelloChrome_120`, and adding `ReadWritePaths=/var/log` to the systemd hardening block so the `-log /var/log/proxy-chain.log` flag can actually write under `ProtectSystem=strict`. **Read before trusting it as your only layer:** the non-Tor/I2P hops are random *public* HTTP proxies scraped from a third-party PAC file — anyone running one of those can see your traffic before it re-enters Tor, so this is defense-in-depth on top of Tor, not a Tor replacement, and it still needs a real Tor (and, if enabled, I2P) daemon already running locally to have anything to chain into. |
