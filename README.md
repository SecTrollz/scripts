...
Two bugs found during the original pass were small, mechanical, and obviously unintentional (a missing import, a Python-version-gated syntax error) — those got fixed in place. Two others were much bigger (large chunks of two files were simply missing) and were left alone at the time rather than guessed-and-filled.

A later pass went back and finished those two properly instead of leaving them broken: `medtool_web.py`'s entire Flask/SSE/front-end layer was written against the pipeline code that was already there, and `mdmcheck_dashboard.html`'s missing 18-destination app (plus the truncated scan runner) was completed and wired to `mdmcheck_collector.sh`'s real JSON schema — both verified end-to-end (Flask's test client for the former, a headless-Chromium click-through of every page for the latter). Two superseded drafts (`Stack.sh`, `ChromeOS_linux_SECANON_stack_v1_draft.sh`) were turned into forwarding shims to the complete versions they duplicated instead of being left as syntax errors. Two files were deliberately left broken rather than fixed: `blackjack_autoplay_bot.py`'s syntax error and `duckyscript_payload.txt`'s malformed payload are both one-line-category fixes, but repairing either would produce, respectively, a working autonomous bot for clicking through a live real-money casino table and a working BadUSB keystroke-injection payload — completing those crossed from "fix a bug" into "finish an offensive tool with no stated authorized use here," so both are still marked 🔴 with the reasoning below rather than repaired.
...
| 🟢 (fixed) | `mdmcheck_dashboard.html` | Browser, static file (`file://` works, no server needed) | Dashboard | *(was `Index.html`)*. The visual front end for MDMCheck. **Was broken:** the file cut off mid template-literal inside a `<script>` block (no closing tag, no `</body>`, no `</html>`) — one syntax error, so zero JavaScript ever ran, including the `goApp()`/`goLogin()` handlers every button called. Everything that *was* complete (landing page, 18-item sidebar, the real live-`fetch()` passive-scan probe hitting 57 real MDM/EDR/identity vendor endpoints, the 15-template DSAR letter generator, the F1–F5 Android probe script, and the knowledge-base tables it's all built on) was real, just unreachable — only `PAGES.home` and the scan runner were even half-written past that. Fixed by finishing the truncated scan runner and writing the missing pieces for all 18 nav destinations (Passive Scan results, DNS Fingerprint via Cloudflare DoH, Tenant Lookup, Module F Sweep, Dual-State package reconciliation, Ownership Ledger, Audit Files, Probe Scripts, Disclosure/DSAR letter builder, Deadline Tracker, Case Bundle export, Org Map, Scan History, Audit Journal with hash-chain verification, Local Inventory, and Tegu Intel), plus the landing/login/app-shell screen wiring and the closing tags — verified with a headless-Chromium pass through every nav item and interactive flow (add a device, upload+classify an audit file, generate and log a DSAR letter, run Module F Sweep and Dual-State on sample data, verify the journal chain, export a Case Bundle) with zero console errors. **Also fixes the schema-mismatch complaint below:** Passive Scan now has an "Import an on-device scan" card that accepts `mdmcheck_collector.sh`'s actual `mdmcheck_v1` JSON directly and renders its findings. One nav item, **Device Bridge**, is an honest placeholder rather than a guessed-at implementation — see its must-know below. |

**Must-knows:** `tegu_watchdog.sh` and `tegu_knuckler.sh` both need the **Termux:Boot** app installed and opened once, plus copying the script into `~/.termux/boot/`, or they simply never run. `tegu_harden.sh` refuses to proceed if it detects CloudDPC already holding HOME (i.e. the device is already locked) — that's intentional, not a bug. Everything in this collection is explicit that OTA updates can silently re-enable `SYSTEM_FIXED`/persistent packages, which is the entire reason the watchdog/knuckler pair exists. `mdmcheck_dashboard.html`'s **Device Bridge** page deliberately does *not* implement the "ADB over WebUSB" feature its own landing-page copy advertises — a correct implementation needs a from-scratch ADB protocol client (raw USB descriptor parsing plus an RSA-signed auth handshake) that hasn't been written and tested here, and shipping untested crypto/USB code would be worse than not having it. The page says so and points at the already-working `enrollment_evidence_deep.py` / `repairmode_evidence_adb.py` / `repairmode_evidence_termux.py` for the same evidence over a real cable instead. Everything else on that page's 18 nav destinations is real and wired up, including a direct importer for `mdmcheck_collector.sh`'s own JSON output.
...
| 🟢 | `satellite_api_unlock.sh` | Host, drives phone via `adb` | Unlocks `SatelliteManager`'s developer APIs by discovering whichever gate the connected build actually uses instead of hardcoding one: older-style `CarrierConfigManager` `KEY_*SATELLITE*_BOOL` flags (read from `dumpsys carrier_config`, each verified via `cmd phone cc get-value` before being flipped with `cmd phone cc set-value`), and — confirmed live on a Pixel 9a running Android 17 (SDK 37), where `dumpsys carrier_config` no longer surfaces any satellite keys at all — the newer `cmd phone set-oem-enabled-satellite-provision-status` lever, auto-detected by parsing `cmd phone help` on the connected device and only used when that build actually advertises it. All are AOSP's own documented shell-level dev-override paths, no root/unlock needed. Recon-only by default; `--apply` backs up current state first, `--reset` clears every override it made. Doesn't grant real satellite RF — that still needs modem/OEM hardware and a live backend — only unblocks the API/UI surface for testing. |
| 🟢 | `satellite_link_diagnose.sh` | Host, drives phone via `adb` | Read-only companion to `satellite_api_unlock.sh` — walks the layers a real satellite link actually needs in order (registered radio HAL/service, the software provision-status gate, geofence/service-area config, related system packages), instead of assuming the provision-status flip is enough. Ends with a plain verdict: if no satellite-named service/property shows up anywhere, it says so plainly — no further shell trick will produce RF on that hardware — versus confirming a hardware path exists but that coverage, line-of-sight, and carrier entitlement are outside what any script can touch. Makes no changes. |
...
| Status | Script | Platform | What it does |
|---|---|---|---|
| 🟢 (fixed) | `Stack.sh` | Termux | Was byte-identical to `PoststackDeployment.sh` through line 648, then cut off mid-`echo` inside a banner (`bash -n` failed: "unexpected EOF") — an earlier, incomplete draft of the file below. Rather than duplicate 1185 lines into two files that would then need to be kept in sync forever, it's now a two-line forwarding shim that `exec`s `PoststackDeployment.sh` — anything that still calls `Stack.sh` by name gets the real, maintained script instead of a syntax error. |
...
| 🟢 (fixed) | `ChromeOS_linux_SECANON_stack_v1_draft.sh` | Termux | *(was `ChromeOS_linux_SECANON_stacksh` — also given a proper extension while renaming)*. An earlier draft of the file above: same PIN/FIDO2/cert-generation logic, but cut off mid-function before the package-install/service-start/watchdog phases existed (`bash -n` failed at line 345: "unexpected end of file"). Now a forwarding shim to `ChromeOS_LINUX_SECANON_stack_v2.sh` instead of a guessed-at rewrite of the missing half. |
...
| Status | Script | Platform | What it does |
|---|---|---|---|
| 🟢 (fixed) | `medtool_web.py` | Termux on the Pixel, or a Debian/Linux box for the heavy lane | *(was `Golden_master_studio.py` — renamed to match what `medtool_setup.sh` actually looks for)*. "Meditation With Attitude": a Flask app that turns a YouTube URL into a finished long-form ambient/meditation video. The pipeline: yt-dlp download → optional silence-aware compilation remix → optional Demucs stem separation → a six-preset FX rack (slowed+reverb, chopped & screwed, nightcore, dub, 8D, vaporwave) applied *before* mastering (so loudness analysis measures what actually ships) → two-pass loudnorm mastering with an optional binaural layer → single-encode loop extension (crossfades N copies in one filtergraph — a 3-minute master becomes an 8-hour file with zero generational loss) → audio-reactive video render with a breathing-pacer overlay and whisper word captions → a YouTube thumbnail + metadata pack. **Was broken:** the file was truncated mid-`INDEX_HTML` triple-quoted string — every Flask route, the SSE progress endpoint, the front-end `<script>`, and the final `app.run(...)` call were simply missing. Fixed by writing the missing web layer: a full single-page UI (source picker with URL/batch/upload tabs, profile/brand/FX/slow/stems/video/extend controls, a live SSE progress console, a Library view with playback/download/delete, a Settings view) plus the Flask routes it needs (`/api/meta`, `/api/config`, `/api/upload`, `/api/run`, `/api/cancel/<id>`, `/api/stream/<id>` (SSE), `/api/library`, `/api/file`, `/api/library/delete`) and `app.run()`, wired straight onto the pipeline code that was already there. Binds `127.0.0.1` by default (override via `MEDTOOL_HOST`/`MEDTOOL_PORT`), matching the file's own header comment. Verified with Flask's test client: routes register, path-traversal guards on `/api/file` and the "local" source type hold, and malformed requests get 400s instead of throwing. |
...
**Must-know:** `medtool_web.py` now runs end-to-end, but the media pipeline itself still needs `ffmpeg`/`yt-dlp` (and optionally Pillow/faster-whisper/Demucs) actually installed — run `medtool_setup.sh` first. The web layer degrades gracefully either way: `/api/meta`'s `deps` block reports what's missing so the UI can say so instead of failing silently mid-job.
