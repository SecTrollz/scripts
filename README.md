...
Across several passes, small mechanical bugs — a missing import, a Python-version-gated syntax error, a wrong fastboot verb, a comparison that could never match, an orchestrator pointed at filenames that don't exist — got fixed in place, the same way a human would fix a typo they noticed while reading. Bugs that would take real judgment calls to fix correctly (large missing chunks of a file, a syntax error inside tooling built specifically to defeat a third party's bot/fraud detection) were left alone and marked 🔴 with the specifics instead of guessed at.
...
| 🟢 (fixed) | `sigint-pydroid.py` | On-device, Termux or Pydroid3 | Self-labeled "SIGINT v3" — same passive LAN-discovery approach as `lan_sigint_scanner.py` ("v2"): ARP-cache read (not spoofed), mDNS/SSDP queries, TCP port fingerprinting. Adds IPv6-dual-stack mDNS/SSDP, widens the liveness sweep to the full fingerprinting port set (v2 used a smaller 10-port subset), and defaults to binding the web UI to `127.0.0.1` instead of v2's every-interface default. Despite the name, "Pydroid3 support" here means the same thing it meant in v2 — it still imports Flask (not preinstalled on Pydroid3, same `pip install` requirement as before) and the only Pydroid3-specific code in the file is a startup banner that prints a warning; there's no separate LAN-only code path. **Was broken:** called `atexit.register(_shutdown)` with no `import atexit` anywhere in the file — a guaranteed `NameError` on every single startup, before the server ever bound its port, on Termux and Pydroid3 alike. Fixed by adding the import. |
| 🟢 | `termux_qemu_debian.sh` | Termux | *(was `Termuxqemudebian.sh`)*. Installs QEMU + ARM64 UEFI firmware in Termux, scrapes the current Debian Bookworm cloud image URL and downloads it, then writes and launches a small Python wrapper that boots it (`virt` machine type, `max` CPU, virtio disk/net, `-nographic`). The from-scratch alternative to `moto_recover_avf.py`'s AVF "Linux Terminal" approach — full QEMU emulation instead of the OS-provided VM, for devices/Android versions without a native Linux Terminal. |

**Which `sigint-pydroid.py` vs `lan_sigint_scanner.py` is stronger, now that the startup crash is fixed:** genuinely close. v3's IPv6 support, wider port sweep, and safer default bind (`127.0.0.1` vs v2's `0.0.0.0`) make it the better default choice on Termux. But it doesn't actually improve Pydroid3 support the way its docstring implies — same Flask dependency, same sockets-only fallback — so a Pydroid3-only user gets no real capability v2 didn't already have. Worth running v3 now that it starts, without expecting the "v3" number to mean a Pydroid3 breakthrough.

---

## 🕵️ Adversary / network-trickery detection

Four Android/Termux scripts checking the same category of thing — is something on this network or this device intercepting, tunneling, or lying to me — and they're a real lineage, not four independent attempts: `detect_network_trickery.py`'s `RISH_PATHS` setup and `run_cmd()` helper are copy-pasted verbatim into `apocalypse_detector.py`, which was forked into `apocalypse_detector_root.py` (v5.11 — whole functions carried over structurally identical, reorganized around a weighted alert system), which was in turn wrapped in a root/rish/no-root privilege-tier abstraction to become `apocalypse_detector_unified.py` (v6.0).

| Status | Script | Platform | What it does |
|---|---|---|---|
| 🟡 | `detect_network_trickery.py` | Android, Termux (`rish` used if available) | The seed of the lineage — VPN/proxy/DNS checks, `RISH_PATHS` privileged-shell locator, user-added CA cert enumeration. Smallest and fastest of the four: no root required, no extra packages, five DNS resolvers cross-checked. **Was broken:** its "is google.com's resolved IP still Google's" check compared a resolved dotted-quad IP string against a list of CIDR-notation strings with plain `in` — which can never match, so the "may be hijacked" warning fired on every single run regardless of the actual result. Fixed with a real `ipaddress` network-containment check. |
| 🟢 | `apocalypse_detector.py` | Android, Termux (`rish` used if available) | "ULTIMATE ADVERSARY DETECTOR" — the richest single-tier version: VPN/proxy/DNS/MITM/tunnel detection, cert cross-checking from 5 resolvers, loopback backdoor scan, IPv6 leak check, ARP poisoning check, NTP tampering check, kernel-level interception check (netfilter/iptables/eBPF), VPN/proxy app fingerprinting via installed-package matching, user-CA enumeration, DNS-over-TLS/private-DNS-mode check. Its own docstring claims cross-checking "5 more [certs] from TOR exit nodes" — no Tor-exit-node code exists anywhere in the file; that claim was never true, in this version or any of its descendants. |
| 🟢 | `apocalypse_detector_root.py` | Android, Termux, root or `rish` | v5.11 — its docstring doubles as a real changelog of bugs found and fixed in earlier versions of itself (a policy-routing allowlist that didn't recognize Android's own "from all unreachable" fail-closed pattern; a hidden-process check whose custom `ps -o pid` column format was silently dropping legitimate vendor/HAL daemons on this device's `ps` build). Both self-documented fixes are real and correctly applied. Restructures detection around a weighted alert-scoring system instead of pass/fail per check. |
| 🟢 | `apocalypse_detector_unified.py` | Android — auto-detects root / `rish` (Shizuku) / no-root-no-rish at startup | v6.0, "The Professional's Choice" — one script, three privilege tiers, each check reports itself unavailable rather than guessing when it needs more access than it has. Adds real anti-spoofing to the DNS checks (`sock.connect()` + transaction-ID verification to reject off-path forged UDP replies, which root.py's DNS query never did) and excludes PIDs already explained by the Termux `LD_PRELOAD` shim from the hidden-process check. Its docstring's claim to contain "every check from all three prior variants" is not accurate — it's missing NTP tampering detection, the DNS-over-TLS/private-DNS check, user-CA enumeration, and VPN/proxy app package fingerprinting, all present in `apocalypse_detector.py` and dropped somewhere between it and `_root.py`. |

**Which is strongest:** `apocalypse_detector_unified.py` for almost everyone — best anti-spoofing logic, cleanest bug history, works whether or not you have root or Shizuku. It does not fully replace `apocalypse_detector.py`, though: if NTP tampering, DoH/private-DNS mode, enumerated user CAs, or VPN-app fingerprinting matter, `apocalypse_detector.py` is the only file here that still checks for them. `apocalypse_detector_root.py` is now strictly subsumed by the unified version — fine to keep for history, nothing functional left that it alone provides. `detect_network_trickery.py`'s niche is speed and zero dependencies (no root, no extra packages) for a quick sanity check, now that its one real bug is fixed.
...
| Status | Script | Platform | What it does |
|---|---|---|---|
| 🟢 | `dns_stack_installer.sh` | Termux | *(was `UnboundDNS_stack_installer.sh`)*. Phase 1, the actual installer: installs Unbound + DNSCrypt-Proxy (source-build fallback if the prebuilt binary isn't available), downloads ad/malware/tracking blocklists (StevenBlack, BlocklistProject) and converts them into Unbound `local-zone` NXDOMAIN entries, configures Unbound on `5335` forwarding to local DNSCrypt-Proxy on `5353` (DNS-over-TLS to Cloudflare/Quad9 as fallback), and generates three more helper scripts: `vpn-setup` (WireGuard/WARP/OpenVPN, ships only a placeholder `YOUR_PRIVATE_KEY_HERE` template — no real keys), `proxy-setup` (tinyproxy:8888, 3proxy SOCKS5:1080/HTTP:3128), `dns-manager` (block/allow/status/test CLI). |
| 🟢 | `dns_stack_postinstall.sh` | Termux | *(was `PoststackDeployment.sh`)*. Phase 2 — its very first check refuses to run unless `$PREFIX/bin/dns-manager` already exists, i.e. it explicitly requires `dns_stack_installer.sh` to have run first. Validates the Unbound config, downloads DNSCrypt resolver/relay lists, checks VPN state, starts the daemons, and writes three persistent CLI utilities — `privacy-status`, `dns-service` (start/stop/restart/enable/disable, autostart via a `.bashrc` hook), `dns-monitor` (live 2-second dashboard). Together with `dns_stack_installer.sh`, this is the one complete, working pipeline in this whole family — run the installer, then this, in that order. |
| 🔴 | `dns_stack_postinstall_draft.sh` | Termux | *(was `Stack.sh`)*. Byte-identical to `dns_stack_postinstall.sh` through line 648, then cuts off mid-`echo` inside a banner (`bash -n` fails: "unexpected EOF"). A truncated leftover save of the file above — not runnable, and superseded by it entirely. |
| 🟡 | `googletv_dns_vpn_proxy_stack.sh` | Termux on a Google TV | *(was `GoogleTVFullNetworkDNSVPNProxy.sh`)*. Meant to be the one-command orchestrator chaining the two files above on a Google TV, and self-describes as "Router Bypass Architecture" (the router only ever sees encrypted VPN traffic, since the resolver/proxy run on the TV itself). **Was broken:** it checked for `./install-complete-stack.sh` and `./complete-working-implementation.sh` before running them — leftover working titles that don't match any filename in this repo — so on a fresh run it printed "Installer script not found" and exited before doing anything. Fixed by pointing both checks at the real `dns_stack_installer.sh`/`dns_stack_postinstall.sh`. |
| 🟢 | `chromeos_dns_gateway.sh` | Chrome OS Crostini (Debian) | *(was `ChromeOS_Linux_DNS_gateway.sh`)*. A genuine platform port, not a relabeled copy — uses `apt-get`, `/etc/...`, `/usr/local/bin`, and systemd units throughout instead of Termux's `pkg`/`$PREFIX` idiom, and refuses to run as root the way a Crostini container script should. Same resolver stack plus optional Tor/I2P, installed as persistent systemd services. **Read before running:** Unbound here binds `0.0.0.0`/`::0` on the standard DNS port `53` (not a high port, not loopback-only) with access-control opened to the detected LAN's `/8` range — this one is built to be a real network-wide gateway other devices point at, not just a personal-device resolver. |
| 🟢 | `termux_secanon_stack_v2.sh` | Termux | *(was `ChromeOS_LINUX_SECANON_stack_v2.sh` — despite the old name, this always ran in Termux, never Chrome OS; renamed to stop implying otherwise)*. "Production hardened" variant, a straight completion of the draft below: PIN (SHA-256 hashed) plus a presence-check it calls FIDO2/U2F (really `lsusb`/`termux-usb-list` detection, not an actual challenge-response) gates first run, generates a self-signed CA/cert chain and stores fingerprints in `.pin` files, then installs Unbound/Tor/I2P/DNSCrypt/tinyproxy/microsocks, a `proot` launcher, and Termux:Boot autostart + a self-healing watchdog. Two of its banner claims don't hold up under reading the code: the generated `proot` launcher is never actually invoked by the `secure-stack start-services` path (services still start un-isolated), and the `.pin` cert fingerprints are written but never read back anywhere to actually pin/verify a connection — both "proot isolation" and "certificate pinning" are scaffolded, not enforced. No real WireGuard/WARP tunnel is ever brought up either, despite "Double VPN" in the banner. |
| 🔴 | `termux_secanon_stack_v1_draft.sh` | Termux | *(was `ChromeOS_linux_SECANON_stack_v1_draft.sh`, originally `ChromeOS_linux_SECANON_stacksh`)*. An earlier draft of the file above, and strictly weaker even where both run: it dies mid-FIDO2-check (`bash -n` fails at line 345, unexpected end of file) before ever installing packages or writing any service config, and its client-cert signing step references an undefined `$client` variable (`"$client-csr.pem"` expands to a bare `-csr.pem` in the working directory) — a real bug independent of the truncation, so even the part that runs doesn't fully work. `termux_secanon_stack_v2.sh` supersedes it feature-for-feature. |

**The real pipeline, if you want this whole stack working:** `dns_stack_installer.sh` then `dns_stack_postinstall.sh`, run manually in that order — that pair is the one thing here that's complete, correct, and actually tested against its own prerequisite check. `googletv_dns_vpn_proxy_stack.sh` is redundant with that pair in intent and is the right one to use *on a Google TV specifically* now that its filename references are fixed; on anything else, just run the pair directly. `chromeos_dns_gateway.sh` earns its own slot for a different platform (Crostini, not Termux) rather than being redundant with anything. `termux_secanon_stack_v2.sh` is a legitimate, more locked-down alternative for PIN/USB-presence-gate/Tor/I2P on top of the same DNS work — genuinely different, not competing — though its proot-isolation and cert-pinning claims are more scaffolding than enforcement today.
...
| Status | Script | Platform | What it does |
|---|---|---|---|
| 🟢 | `blackjack-advisor.user.js` | Tampermonkey/Violentmonkey userscript, tuned for Firefox for Android | v1.7.0 (version number unchanged, but the file has picked up real fixes since the last pass). Glassmorphism overlay: DOM-heuristic table reading (with a pasted-plugin system for site-specific adapters and calibrated canvas pixel-matching), Hi-Lo count, basic strategy + count deviations, Kelly-fraction bet sizing, an accessibility output mode, and a "provably fair" audit panel that talks to the companion server below. Advisory only — never clicks anything or sets bet fields. Domain-gated with a per-hostname confirmation dialog. Recent fixes: a split-hand index could drift out of sync with the filtered hand list when an earlier hand slot was empty (now resolved against the same filtered list it indexes into); the accessibility signature only fingerprinted the recommendation text, so two different hands that both happened to say "HIT, basic strategy" wouldn't both announce (now includes the hand itself); and canvas-only (non-DOM) tables were silently missing from the provably-fair hand history since canvas classification only ever identifies rank, not suit (now logged with an explicit unknown-suit marker instead of dropped). |
...
| `ChromeOS_linux_SECANON_stacksh` → `ChromeOS_linux_SECANON_stack_v1_draft.sh` | `termux_secanon_stack_v1_draft.sh` |
| `Blah.sh` | `banking_denylist_setup.sh` |
| `SafeIDupdater.sh` | `moto_identifier_changer.sh` |
| `Termux_safeIDupdater.sh` | `moto_identifier_changer_termux.sh` |
| `fix.sh` | `fastboot_adb_bootloop_fix.sh` |
| `Termuxqemudebian.sh` | `termux_qemu_debian.sh` |
| `Apocalypse_detector.py` | `apocalypse_detector.py` |
| `Apocalypse_Detector_root.py` | `apocalypse_detector_root.py` |
| `Apocalypse_detector_unified.py` | `apocalypse_detector_unified.py` |
| `UnboundDNS_stack_installer.sh` | `dns_stack_installer.sh` |
| `PoststackDeployment.sh` | `dns_stack_postinstall.sh` |
| `Stack.sh` | `dns_stack_postinstall_draft.sh` |
| `GoogleTVFullNetworkDNSVPNProxy.sh` | `googletv_dns_vpn_proxy_stack.sh` |
| `ChromeOS_LINUX_SECANON_stack_v2.sh` | `termux_secanon_stack_v2.sh` |
| `ChromeOS_Linux_DNS_gateway.sh` | `chromeos_dns_gateway.sh` |
