 #!/usr/bin/env python3
"""
SIGINT v2 — Local Network & RF Asset Locator
Single file. Termux (primary) / Pydroid3 (degraded). No root.

TARGET ENVIRONMENT
    Termux is the correct host. Pydroid3 is NOT fatal — Tier 1 below is pure
    sockets and runs there fine — but you lose WiFi RSSI, GPS and Bluetooth,
    because the termux-* CLI bridge belongs to Termux's APK and is not on
    Pydroid3's PATH. The capability panel tells you exactly what you lost.

CAPABILITY TIERS
  TIER 1 — LAN ASSET INVENTORY                        [pure sockets, always]
    ICMP+TCP liveness prefilter -> ARP harvest -> deep TCP fingerprint,
    fused with SSDP M-SEARCH and full mDNS (with TXT/SRV record parsing).
    TXT records are the payoff: Chromecast publishes md= (model) and
    fn= (friendly name); AirPlay publishes model=; printers publish ty=.
    This yields exact model strings, not guesses.

  TIER 2 — WIFI RF SURVEY                    [termux-api + Termux:API + loc]
    termux-wifi-scaninfo -> BSSID / SSID / RSSI / band.
    Proximity buckets and warmer/colder trend. These are APs, not phones.

  TIER 3 — BLUETOOTH                                  [usually unavailable]
    Official termux-api has no bluetooth scan. Only an unofficial fork
    adds termux-bluetooth-scaninfo. Probed functionally, never assumed.

NOT POSSIBLE, therefore absent:
    Counting phones (MAC randomization rotates unpaired addresses every few
    minutes). Person detection. Lat/lon fixes from RSSI (indoor error
    exceeds a house). This tool reports proximity buckets and trend.
...
"APK missing" from "permission denied" from "location services off", and
prints the specific remedy for each. Check it before trusting an empty scan.
"""
...
SCHEMA_VERSION = 2
SIGHTINGS_PER_ASSET = 120  # retention cap; pruned after every scan

# ============================================================================
# PREFLIGHT — functional probes, not shutil.which theatre.
#
# shutil.which("termux-wifi-scaninfo") returning a path proves only that the
# CLI shim is installed. The shim talks to the Termux:API Android app over an
# intent; if that APK is absent the command exits silently or hangs. That is
# the single most common silent failure in this whole stack, so every termux
# capability here is verified by actually running it and classifying what
# comes back.
# ============================================================================
...
def classify_termux_result(name, rc, out, err, timed_out):
    """
    Turn a termux-* invocation into a status + a specific remedy.
    Statuses: ok | no_binary | no_apk | no_permission | location_off | error
    """
    if err == "not found":
        return {
            "status": "no_binary",
            "detail": name + " is not installed",
            "remedy": "pkg install termux-api   (then also install the "
                      "Termux:API app from F-Droid)",
        }
    if timed_out:
        return {
            "status": "no_apk",
            "detail": name + " hung with no response",
            "remedy": "The Termux:API *app* is missing or was force-stopped. "
                      "Install 'Termux:API' from F-Droid, open it once, then "
                      "retry. The pkg alone is not enough.",
        }
    if not out:
        return {
            "status": "no_apk",
            "detail": name + " returned nothing",
            "remedy": "Install the Termux:API app from F-Droid and open it "
                      "once so Android registers it.",
        }
    ...
    if "api_error" in low or "permission" in low or "denied" in low:
        if "location" in low:
            return {
                "status": "location_off",
                "detail": out[:150],
                "remedy": "Android returns an empty WiFi scan when location "
                          "services are OFF, even with permission granted. "
                          "Turn on Settings > Location, then grant Termux "
                          "location permission.",
            }
        return {
            "status": "no_permission",
            "detail": out[:150],
            "remedy": "Android Settings > Apps > Termux > Permissions > "
                      "Location > Allow (choose 'While using the app' at "
                      "minimum). Then re-run preflight.",
        }
...
def preflight(deep=True):
    """
    Establish real capabilities. `deep=False` skips the live invocations for
    a fast startup; the UI calls deep=True.
    """
    checks = {}

    # --- pure-socket tiers: no external dependency, always available ---
    checks["lan"] = {
        "status": "ok",
        "detail": "pure sockets (ICMP/TCP/SSDP/mDNS)",
        "remedy": "",
    }

    # --- helper binaries that improve Tier 1 but are not required ---
    for binname, label in (("ping", "icmp_prefilter"), ("ip", "arp_iproute")):
        checks[label] = (
            {"status": "ok", "detail": binname + " present", "remedy": ""}
            if shutil.which(binname)
            else {
                "status": "no_binary",
                "detail": binname + " missing",
                "remedy": "pkg install " + ("inetutils" if binname == "ping"
                                            else "iproute2")
                          + "  (optional — sweep still works, just slower "
                            "and without MAC harvest)",
            }
        )

    # --- termux bridge: verify functionally ---
    termux_checks = [
        ("wifi_scan", "termux-wifi-scaninfo", 22),
        ("wifi_conn", "termux-wifi-connectioninfo", 12),
        ("gps", "termux-location", 4),   # short: we only want the failure mode
        ("bluetooth", "termux-bluetooth-scaninfo", 8),
    ]
    for key, cmd, tmo in termux_checks:
        if not shutil.which(cmd):
            checks[key] = {
                "status": "no_binary",
                "detail": cmd + " not installed",
                "remedy": (
                    "Official termux-api provides no bluetooth scan. This is "
                    "expected, not a misconfiguration."
                    if key == "bluetooth"
                    else "pkg install termux-api, plus the Termux:API app "
                         "from F-Droid."
                ),
            }
            continue
        if not deep:
            checks[key] = {"status": "unverified",
                           "detail": "binary present, not yet exercised",
        ...
        # A GPS timeout is normal (waiting for a fix), not an APK failure.
        if key == "gps" and to:
            checks[key] = {
                "status": "unverified",
                "detail": "no fix within 4s (normal indoors)",
                "remedy": "Use the Fix GPS button; allow up to 30s.",
            }
        else:
            checks[key] = classify_termux_result(cmd, rc, out, err, to)

    checks["wake_lock"] = (
        {"status": "ok", "detail": "termux-wake-lock present", "remedy": ""}
        if shutil.which("termux-wake-lock")
        else {"status": "no_binary", "detail": "termux-wake-lock missing",
              "remedy": "pkg install termux-tools — without it Android Doze "
                        "will throttle background scans."}
    )
...
# ============================================================================
# WAKE LOCK — Android Doze will throttle or stall background scanning.
# Held only while auto-scan is armed, and always released.
# ============================================================================
...
# ============================================================================
# INTERFACE ENUMERATION — VPN-aware.
#
# Connecting a UDP socket to 8.8.8.8 reveals the route the kernel would take,
# which on a VPN is the tunnel, not the LAN you want to inventory. So we
# enumerate every IPv4 interface, flag tunnels, and prefer a real wlan/eth.
# ============================================================================
...
def enumerate_interfaces():
    """Return list of {iface, ip, cidr, is_vpn, hosts}."""
    results = []
    if shutil.which("ip"):
        ...
        for line in out.splitlines():
            ...
            if iface == "lo" or ip.startswith("127."):
                continue
            try:
                net = ipaddress.ip_network(ip + "/" + str(plen), strict=False)
            except ValueError:
                continue
            results.append({
                "iface": iface, "ip": ip, "cidr": str(net),
                "is_vpn": bool(VPN_IFACE_RE.match(iface)),
                "hosts": net.num_addresses - 2 if net.num_addresses > 2 else 1,
            })

    if not results:
        # Fallback: route-discovery. Assume /24, and say so.
        try:
...
def choose_scan_target(prefer_iface=None):
    """
    Pick which subnet to inventory. Non-VPN physical interfaces win, because
    the LAN you care about is behind the WiFi adapter, not the tunnel.
    Oversized prefixes are clamped to the /24 around our own address.
    """
    ...
    # Prefer wlan/eth by name, then smallest host count (most specific).
    def rank(i):
    ...
    if net.num_addresses > 1024:
        net = ipaddress.ip_network(chosen["ip"] + "/24", strict=False)
        clamped = True

    meta = {
        "interfaces": ifaces,
        "chosen_iface": chosen["iface"],
        "self_ip": chosen["ip"],
        "subnet": str(net),
        "clamped_from": chosen["cidr"] if clamped else None,
        "vpn_present": any(i["is_vpn"] for i in ifaces),
        "vpn_skipped": [i["iface"] for i in ifaces
                        if i["is_vpn"] and i["iface"] != chosen["iface"]],
...
# ============================================================================
# mDNS — proper DNS message parsing, including compression pointers.
#
# The v1 parser scraped length-prefixed labels and ignored 0xC0 pointers, so
# compressed responses (most of them) lost names. This parses records for
# real, which additionally gives access to TXT rdata — where Chromecast puts
# md=<model> and fn=<friendly name>, AirPlay puts model=, printers put ty=.
# ============================================================================
...
def _parse_dns_name(data, offset, depth=0):
    ...
    Returns (name, offset_after_this_name).
    """
    labels = []
    n = len(data)
    jumped = False
...
def _parse_txt(rdata):
    """TXT rdata is a sequence of length-prefixed key=value strings."""
    kv = {}
    i = 0
    while i < len(rdata):
        ln = rdata[i]
...
def parse_mdns_message(data):
    """
    Parse an mDNS response into structured records.
    Returns {"ptr": [...], "txt": {name: kv}, "srv": {name: target},
             "a": {name: ip}, "instances": set()}
    """
...
def mdns_discover(timeout=4):
    """Multicast PTR queries, then structured parse of everything returned."""
    results = {}
    try:
...
# TXT keys that carry a real model or human-assigned name.
TXT_MODEL_KEYS = ["md", "model", "am", "ty", "product", "usb_mdl", "mdl"]
TXT_NAME_KEYS = ["fn", "n", "name", "friendlyname", "note"]


def txt_extract(txt_map):
    """Pull the best model + friendly name out of all TXT records for a host."""
    model, fname = "", ""
...
def ssdp_discover(timeout=4):
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            e = results.setdefault(addr[0], {"server": "", "location": "", "st": ""})
...
# ============================================================================
# LIVENESS: ICMP + light TCP prefilter, then ARP harvest, then deep fingerprint
#
# v1 deep-probed all 254 addresses, and silently missed any host that was up
# but had none of the probed ports open. Pinging first both finds those hosts
# and populates the kernel ARP cache, which is where the MACs come from — and
# MACs give us stable asset identity across DHCP lease changes.
# ============================================================================
...
LIVENESS_PORTS = [80, 443, 8008, 7000, 8060, 22, 445, 1400, 631, 32400]
...
def icmp_alive(ip, timeout=1):
    """Unprivileged ICMP via the ping binary. Also populates the ARP cache."""
...
def find_live_hosts(net, extra_ips=()):
    """
    Two-stage liveness. ICMP catches hosts with no open ports; the light TCP
    pass catches hosts that drop ICMP. Union of both, plus SSDP/mDNS speakers.
    """
...
def fingerprint_hosts(live_ips):
    """Deep port fingerprint, only against confirmed-live hosts."""
    found = {}
    lock = threading.Lock()
...
# ============================================================================
# Reverse DNS with a hard bound.
#
# socket.gethostbyaddr uses the system resolver and honours no Python-level
# timeout; a broken resolver can block for many seconds per host and freeze a
# scan. Bounded in a daemon thread and abandoned if slow.
# ============================================================================
...
SERVER_MODEL_HINTS = [
    (r"Roku", "tv", "Roku"),
    (r"webOS", "tv", "LG webOS TV"),
    (r"Samsung", "tv", "Samsung"),
    (r"BRAVIA|Sony", "tv", "Sony BRAVIA"),
    (r"Chromecast|Eureka", "tv", "Chromecast / Google TV"),
    (r"Sonos", "speaker", "Sonos"),
    (r"AirTunes|AirReceiver|AppleTV|Apple ?TV", "tv", "Apple TV / AirPlay"),
    (r"AirPort", "router", "Apple AirPort"),
    ...
    (r"Xbox", "console", "Xbox"),
    (r"PlayStation|PS4|PS5", "console", "PlayStation"),
    (r"Plex", "server", "Plex"),
...
MDNS_SERVICE_HINTS = {
    ...
    "_airplay": ("tv", "AirPlay device"),
    "_raop": ("speaker", "AirPlay audio"),
    ...
    "_device-info": ("unknown", ""),
}


def _name_score(n):
    """Prefer human names ("Living Room TV") over generated IDs."""
    score = 0
    if " " in n:
        score += 10
...
def stable_asset_id(ip, mac):
    """
    MAC-first identity. v1 keyed LAN assets on IP, so a DHCP lease change
    made a known device look brand new against the baseline.
    """
    if mac:
        return "lan:mac:" + mac
    return "lan:ip:" + ip
...
def classify_lan_host(ip, sweep_entry, ssdp_entry, mdns_entry, arp_mac,
                      do_rdns=True):
    dtype, model, name = "unknown", "", ""
    evidence = []

    # --- mDNS TXT records: the highest-fidelity source available ---
    if mdns_entry:
    ...
    if sweep_entry:
        ...
        if sweep_entry.get("ports"):
            evidence.append("ports:" + ",".join(
                str(p) for p in sweep_entry["ports"][:8]))
    ...
    if not name and do_rdns:
        rd = reverse_dns(ip, timeout=1.0)
        if rd:
            name = rd[:48]
            evidence.append("rdns")
    if not name:
        name = model or ("host-" + ip.split(".")[-1])
...
def scan_lan(prefer_iface=None):
    ...
    ssdp = ssdp_discover(timeout=4)
    mdns = mdns_discover(timeout=4)

    speakers = set(ssdp) | set(mdns)
    live = find_live_hosts(net, extra_ips=speakers)
    live.discard(meta.get("self_ip"))

    arp = read_arp_table()          # populated by the ping stage above
    sweep = fingerprint_hosts(sorted(live))

    assets = []
    for ip in sorted(live, key=lambda x: tuple(int(p) for p in x.split("."))
                     if re.fullmatch(r"[\d.]+", x) else (0,)):
        assets.append(classify_lan_host(
            ip, sweep.get(ip), ssdp.get(ip), mdns.get(ip), arp.get(ip)))
...
# ============================================================================
# STORAGE — with retention pruning
# ============================================================================
...
def init_db():
    with db() as c:
        ...
        if ver != SCHEMA_VERSION:
            # v1 keyed LAN assets by IP; those baseline rows are meaningless
            # under MAC-based identity, so drop them rather than false-flag.
...
def prune_db(keep_per_asset=SIGHTINGS_PER_ASSET):
    """Bound growth: keep only the newest N rows per asset."""
    with db() as c:
        c.execute(
...
def do_full_scan():
    ...
    with _lock:
        STATE.update({
            "assets": assets, "lan_meta": lan_meta,
            "channel_errors": {"wifi": wifi_err, "bluetooth": bt_err,
                               "lan": lan_meta.get("error")},
            "last_scan": time.time(), "phase": "idle", "pruned": pruned,
        })
        STATE["scan_count"] += 1
    return assets
...
var CAP_ORDER=[['lan','LAN'],['icmp_prefilter','ICMP'],['arp_iproute','ARP'],
...
function poll(){
  fetch('/api/state').then(function(r){return r.json();}).then(function(s){
    ...
    renderNet(s);
    // Only refetch the asset list when a scan actually completed.
    if(s.last_scan!==lastScan){lastScan=s.last_scan;
...
function schedule(s){
  ...
  // Back off when nothing is happening instead of hammering every 4s.
  var ms = (s && s.phase!=='idle') ? 2000 : ((s && s.running) ? 6000 : 15000);
...
document.getElementById('once').addEventListener('click',function(){
  ...
  post('/api/scan/once').then(function(){lastScan=null;poll();})
...
document.getElementById('base').addEventListener('click',function(){
  post('/api/baseline/set').then(function(){lastScan=null;poll();});});
document.getElementById('baseclr').addEventListener('click',function(){
  post('/api/baseline/clear').then(function(){lastScan=null;poll();});});
...
if __name__ == "__main__":
    ...
    CAPS, PREFLIGHT = preflight(deep=False)
    init_db()

    print("")
    print("  SIGINT v2 — local network & RF asset locator")
    print("  " + "-" * 52)
    for k, label in (("lan", "LAN tier"), ("icmp_prefilter", "ICMP prefilter"),
                     ("arp_iproute", "ARP/iproute"), ("wifi_scan", "WiFi RSSI"),
                     ("gps", "GPS"), ("bluetooth", "Bluetooth"),
                     ("wake_lock", "Wake lock")):
        c = PREFLIGHT.get(k, {})
        print("   %-15s %-11s %s" % (label, c.get("status", "?"),
                                     c.get("detail", "")[:44]))
    ifs = enumerate_interfaces()
    for i in ifs:
        print("   iface           %-11s %s%s" % (
            i["iface"], i["cidr"], "  (vpn)" if i["is_vpn"] else ""))
    if not ifs:
        print("   iface           none        no IPv4 interface found")
    print("  " + "-" * 52)
    print("   Startup probe is shallow. Hit 'Preflight' in the UI to")
    print("   functionally verify the termux bridge and get exact remedies.")
    print("  " + "-" * 52)
    print("   db      " + DB_PATH)
    print("   open    termux-open http://localhost:%d" % PORT)
    print("")
